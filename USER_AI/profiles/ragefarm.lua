...
Profile = {}
-- Useful functions
function isAttackable(myid, mob_data) -- Depends on mob's target
  return mob_data.target == myid or ((mob_data.target == 0 or mob_data.target == -1 or isMob(mob_data.target)))
end
...
ftran[stIdle] =
  function (myid)
    ...
    local actors = getActors(V_TARGET, V_ATTACKRANGE, 
      function (act, id)
...
ftran[stFollowTarget] =
  function (myid, target, memory)
    ...
    local actors = getActors(V_TARGET, V_HOMUNTYPE,
	  ...
	  if(id ~= target and isAttackable(myid, act[id]) and getDistance2(myid, id) < getDistance2(myid, target) and not memory[owner] and memory[id] == nil) then
	    return {stFollowTarget, {myid, id, memory}}
	  end
    ...
    local actors = getActors(V_TARGET, V_HOMUNTYPE, V_ATTACKRANGE,
	  ...
	  if(id ~= target) then
	    ...
	    if(t_motion and memory[id] ~= nil and (act.hits == nil or memory[id] < act.hits)) then
	      if memory[target] <= 0 and memory[id] > 0 then
	        memory[id] = 0
	      else
